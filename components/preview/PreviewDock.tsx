...
import { useAIConfigStore, AI_MODEL_CONFIGS } from "@/store/useAIConfigStore";
...
const PreviewDock = ({
  ...
  const handleGrammarCheck = useCallback(async () => {
    toast.warning("Kiểm tra ngữ pháp đang được phát triển.");
    ...
    //   toast.error(
    //     <>
    //       <span>Vui lòng cấu hình AI trước</span>
    //       <Button
        ...
        <TooltipProvider delayDuration={0}>
          <Dock>
            <div className="flex flex-col gap-2">
              <DockIcon>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div
                      className={cn(
                        "flex cursor-pointer h-6 w-6 items-center justify-center rounded-lg",
                        "hover:bg-orange/10 hover:text-orange dark:hover:bg-orange/20 transition-all duration-200"
                      )}
                    >
                      <TemplateSheet />
                    </div>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div
                      className={cn(
                        "flex cursor-pointer h-6 w-6 items-center justify-center rounded-lg",
                        "hover:bg-orange/10 hover:text-orange dark:hover:bg-orange/20",
              ...
              <div className="w-full h-[1px] bg-gray-200 dark:bg-neutral-700" />
              <DockIcon>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className={cn(
                        ...
                        !sidePanelCollapsed && [
                          ...
                          "hover:bg-orange/90 dark:hover:bg-orange/90 hover:text-white",
                          "shadow-sm"
                        ]
                      )}
                    >
                      {sidePanelCollapsed && <PanelRightClose size={18} />}
                      {!sidePanelCollapsed && <PanelRightOpen size={18} />}
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="left" sideOffset={10}>
                    <p>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className={cn(
                        "flex h-[28px] w-[28px] cursor-pointer items-center justify-center rounded-lg transition-all",
                        "hover:bg-orange/10 hover:text-orange dark:hover:bg-orange/20",
                        "active:scale-95",
                        !editPanelCollapsed && [
