    ...
    >
      {showControls && allItemIds.length > 0 && (
        <div className="mb-4 flex items-center justify-end gap-2">
          <button
            onClick={expandAll}
            className="text-xs font-semibold text-[var(--orange)] border border-[var(--orange)]/20 px-3 py-1 rounded-full hover:bg-[var(--orange)]/10"
            type="button"
          >
            Mở tất cả
          </button>
...
export const AccordionItem: React.FC<AccordionItemProps> = ({
  ...
  return (
    <div
      ...
      data-active={active}
      className={cn(
        "overflow-hidden rounded-3xl border border-[var(--border)] bg-white/90 shadow-[var(--shadow-ring)] transition-all duration-200",
        active ? "shadow-[0_22px_45px_-30px_rgba(242,148,39,0.35)]" : "",
        className,
      )}
    >
      {children}
...
export const AccordionHeader: React.FC<AccordionHeaderProps> = ({
  ...
  return (
    <button
      ...
      className={cn(
        "flex w-full items-center justify-between gap-4 px-6 py-4 text-left text-sm font-medium text-[var(--deep-blue)] hover:bg-[var(--deep-blue)]/5",
        className,
      )}
